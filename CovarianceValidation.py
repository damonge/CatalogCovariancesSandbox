...
case = "catalog_spin0_allsame"
plot_dir = f"/plots/{case}"
out_dir = "."
...
print("Generating mock data fields")
...
fields_dat_uniq = [ut.get_field(lmax, types[i], nside_corr=None, **(data[i]))
                   for i in list(set(field_ids))]
fields_dat = [fields_dat_uniq[i] for i in field_ids]

print("Initializing power spectrum calculator")
...
print("Subtract shot noise?", fields_dat[0] is fields_dat[2])
...
fname = f'{plot_dir}/tmp_fullimp_cls_nsims{nsims}.npz'
if not os.path.isfile(fname) or overwrite:
    ...
    for i in range(nsims):
        ...
        # For all fields
        fields_uniq = [ut.get_field(lmax, types[i], nside_corr=None,
                                    **(data[i]))
                       for i in list(set(field_ids))]
        ...
        if "mom" in types or "num" in types:
            ...
            w = wsps_dat
        cls.append([[w[i][j].decouple_cell(pcl[i, j])
                     for j in range(4)] for i in range(4)])
    pcls = np.array(pcls)
    cls = np.array(cls)
    np.savez(fname, pcls=pcls, cls=cls)
d = np.load(fname)
pcls = d["pcls"]
cls = d["cls"]
